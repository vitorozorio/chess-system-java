...
public class Board {
    ...
    public Board(int rows, int columns) {
        if(rows <1 || columns <1) {
            throw new BorderException("Erro ao Criar o tabuleiro: É nescessario que ajá ao menos 1 linha e uma coluna");
        }
        this.rows = rows;
    ...
    public void placePiece(Piece piece, Position position){
        if(thereIsAPiece(position)){
            throw new BorderException("there is already a piece on position " + position );
        }
